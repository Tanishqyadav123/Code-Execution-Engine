...
function page() {
  ...
  const markDown = useRenderMarkdown({
    ...
    statement: form.getValues("statement"),
    testCases: form.getValues("testCases"),
  });

  const router = useRouter();

  console.log(showMarkDown, " Showing the marks down");
  ...
  return (
    <div className=" flex  gap-5 h-screen w-[90vw] ">
                ...
                <div className="grid gap-2">
                  <Controller
                    ...
                    render={({ field, fieldState }) => {
                      return (
                        <Field>
                          <FieldLabel htmlFor="difficultyLevel">
                            Level
                          </FieldLabel>
                          <CommonSelect
      ...
      <div className=" mt-16 flex flex-col items-center w-[10vw] gap-2">
        <Switch
          checked={showMarkDown}
          onClick={() => setShowMarkDown(!showMarkDown)}
        />
        <span className="text-xs font-bold">(See Markdown)</span>
      </div>
    </div>
  );
}
